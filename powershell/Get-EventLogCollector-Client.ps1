<#
.SYNOPSIS
    Event Log Collector für Windows 11 Clients – AdminSheet Event Log Analyzer
...
.DESCRIPTION
    ...
    Gesammelte Bereiche:
    - Systemfehler & Abstürze (Kernel-Power, WER, BugCheck)
    - Anwendungsfehler & Hänger
    - Netzwerk & WLAN Probleme
    - Treiber & Hardware Fehler
    - Energie & Standby Probleme
    - Sicherheit (Anmeldungen, gesperrte Konten)
...
.NOTES
    Autor: AdminSheet
    Version: 1.0
    Benötigt: PowerShell 5.1+, Admin-Rechte für Security-Log
...
function Collect-Events {
    ...
    foreach ($e in $events) {
        ...
        $script:Results += [PSCustomObject]@{
            ...
            Category    = $Category
            Message     = ($e.Message -replace '\r?\n',' ' -replace '\s+',' ').Substring(0, [Math]::Min(300, ($e.Message -replace '\r?\n',' ').Length))
        }
    }
...
# ── ABSTÜRZE & NEUSTARTS ──────────────────────────────────
Write-Host "► Abstürze & Neustarts" -ForegroundColor Yellow
...
Collect-Events 'Application' @(1000,1001,1002)         @(2,3)    'Anwendungsabsturz/-hänger'           'Absturz'

# ── SYSTEM & TREIBER ─────────────────────────────────────
Write-Host "► System & Treiber" -ForegroundColor Yellow
...
Collect-Events 'System'      @(55,98,153)              $null     'Datenträger/Dateisystem Fehler'       'System'
Collect-Events 'System'      @(2004,2013,2019,2020)    $null     'Speicher/Pool Fehler'                 'System'
Collect-Events 'System'      @(1,3,6,7,14,15)          @(1,2)    'Allgemeine kritische Systemfehler'    'System'

# ── NETZWERK & WLAN ──────────────────────────────────────
Write-Host "► Netzwerk & WLAN" -ForegroundColor Yellow
...
Collect-Events 'System'      @(5719,5722)              $null     'Domänen-/Netzwerkanmeldung fehlgesch' 'Netzwerk'
Collect-Events 'Microsoft-Windows-NetworkProfile/Operational' @(10000,10001) $null 'Netzwerkprofil geändert' 'Netzwerk'

# ── ENERGIE & STANDBY ────────────────────────────────────
Write-Host "► Energie & Standby" -ForegroundColor Yellow
...
Collect-Events 'System'      @(43)                     $null     'USB Gerät nicht erkannt'              'Hardware'
